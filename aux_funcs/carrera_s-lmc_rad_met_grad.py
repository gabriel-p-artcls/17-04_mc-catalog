...
def get_data(name_data):
    ...
    if name_data == 'carrera_11':
        ...
        dec_lst = ['-66d48m', '-65d03m', '-63d33m', '-71d00m', '-61d59m',
                   '-71d10m', '-61d00m', '-71d00m', '-60d00m', '-71d00m']
        inc, pa = Angle('34.7d'), Angle('189.3d')  # LMC
        fe_h = np.array([-0.58, -0.64, -0.54, -0.43, -1.03, -0.74, -1.04,
                         -0.87, -0.99, -0.85])
        sigma_feh = np.array([0.47, 0.54, 0.48, 0.39, 0.64, 0.62, 0.70, 0.56,
                              0.20, 0.35])
    elif name_data == 'carrera_08':
        """
        Data taken from Carrera et al. (2008), "The Chemical Enrichment History
        of the Small Magellanic Cloud and Its Gradients"

        Inclination and position angle data taken from Cioni et al. (2009),
        "The Chemical Enrichment History of the Small Magellanic Cloud
        and Its Gradients"

        Center taken from Noel et al. (2007)
        (https://ui.adsabs.harvard.edu/#abs/2007AJ....133.2037N/abstract)
        which is 'Paper I' according to Carrera et al. (2008).
        """
        ...
        ra_lst = ['00h57m', '00h37m', '00h36m', '01h11m', '01h12m', '00h35m',
                  '01h16m', '01h00m', '00h47m', '00h33m', '00h49m', '01h02m',
                  '00h53m']
        dec_lst = ['-73d53m', '-72d18m', '-72d25m', '-72d49m', '-72d36m',
                   '-72d01m', '-72d59m', '-74d57m', '-75d30m', '-70d28m',
                   '-75d44m', '-75d46m', '-76d46m']
        fe_h = np.array([-1.01, -0.95, -0.98, -1.08, -1.16, -1.09, -0.96,
                         -1.07, -1.15, -1.58, -1.00, -1.29, -1.64])
        sigma_feh = np.array([0.33, 0.17, 0.25, 0.21, 0.32, 0.24, 0.26, 0.28,
                              0.27, 0.57, 0.28, 0.42, 0.50])
    elif name_data == 'palma_15':
        '''
    ...
    return cent, dist, inc, pa, ra_lst, dec_lst, fe_h, sigma_feh
