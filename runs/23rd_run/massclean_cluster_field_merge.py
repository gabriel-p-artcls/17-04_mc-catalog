...
def get_clust_names():
    ...
    sc_path = join(mypath + '/massclean2.014/clusters/')
...
def exp_func(x):
    ...
    '''
    a, b, c = 2.e-05, 0.37, 0.015
    return min(0.4, a * np.exp(b * x) + c)
...
def make_plots(plot_params):
    ...
    # Set plot limits
    plt.xlim(min(mag_data_f) - 0.5, max(mag_data_f) + 0.5)
    plt.ylim(-0.005, max(e_mag_data_f) + 0.1)
    # Set axis labels
    ...
    direc = join(out_path_sub, 'figs/')
    if not exists(direc):
        makedirs(direc)
    plt.savefig(join(direc, str(clust_name) + '.png'), dpi=150)
