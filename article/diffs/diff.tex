...
%DIF DEL perren_v04.tex   Thu Aug 11 15:30:56 2016
%DIF ADD perren_v05.tex   Thu Aug 11 15:30:57 2016
...
of clusters, with the variation in the \DIFdelbegin \DIFdel{x axis }\DIFdelend \DIFaddbegin \DIFadd{x-axis }\DIFaddend proposed by
...
plot. In the original BA plot the default \DIFdelbegin \DIFdel{x axis }\DIFdelend \DIFaddbegin \DIFadd{x-axis }\DIFaddend displays the mean values
...
%
The BA plot shows differences in the \DIFdelbegin \DIFdel{y axis }\DIFdelend \DIFaddbegin \DIFadd{y-axis }\DIFaddend in the sense $\Delta$= (\texttt
{ASteCA} minus literature), versus values found by the code in the \DIFdelbegin \DIFdel{x axis}\DIFdelend \DIFaddbegin \DIFadd{x-axis}\DIFaddend . Errors
...
\emph{Center}: ``delta'' plot, showing the differences between reddening
...
\caption{\DIFdelbeginFL \emph{\DIFdelFL{Top}}%DIFAUXCMD
...
curves.
\DIFdelbeginFL \emph{\DIFdelFL{Bottom}}%DIFAUXCMD
...
\section{\DIFdelbegin \DIFdel{Distribution of fundamental }\DIFdelend \DIFaddbegin \DIFadd{Fundamental }\DIFaddend parameters \DIFaddbegin \DIFadd{in the analyzed database}\DIFaddend }
\label{sec:param-dist}

We present here a brief summary of the distribution of \DIFdelbegin \DIFdel{the values }\DIFdelend \DIFaddbegin \DIFadd{values in the parameters
space, }\DIFaddend for the five fundamental parameters obtained with \texttt{ASteCA}, for
\DIFdelbegin \DIFdel{our set of }\DIFdelend \DIFaddbegin \DIFadd{the }\DIFaddend 239 Magellanic Clouds clusters \DIFdelbegin \DIFdel{. The metallicity and age parameters cover a wide
range in both galaxies, and their distribution can be regarded as a
representative randomized sample of the cluster system in the
MCs.}\DIFdelend \DIFaddbegin \DIFadd{in our catalog.
}

\DIFadd{A method is devised to allow an unbiased analysis of the estimated values in the
parameters space, shown in Sect.~\ref{ssec:kde_method}.
}\DIFaddend %
\DIFdelbegin \DIFdel{The rest of the fitted parameters -- }\DIFdelend \DIFaddbegin \DIFadd{This method is applied on the five fitted parameters, and the results presented
in Sect.~\ref{ssec:dist_ranges}. The parameters }\DIFaddend reddening, distance, and total
mass\DIFdelbegin \DIFdel{-- }\DIFdelend \DIFaddbegin \DIFadd{, }\DIFaddend do not span \DIFdelbegin \DIFdel{a large enough range}\DIFdelend \DIFaddbegin \DIFadd{sufficiently large ranges to account for the values found
in the MCs}\DIFaddend . Their distribution \DIFdelbegin \DIFdel{thus can }\DIFdelend \DIFaddbegin \DIFadd{can thus }\DIFaddend only be though to characterize the state
of those star clusters that compose this particular catalog.
%DIF > 
\DIFaddbegin \DIFadd{The metallicity and age parameters, on the other hand, cover a wide range in
...
\DIFdelbegin \DIFdel{Figs.~\ref{fig:kde_fig0} and~\ref{fig:kde_fig1} show 1D and 2D density maps
constructed via Eqs.~\ref{eq:kde-1d} and~\ref{eq:kde-2d}, for two paired
parameters, for each of the MCs.}%DIFDELCMD < \@
%DIFDELCMD < %%%
\DIFdelend %
...
\subsection{\DIFadd{Distribution of parameters within the observed ranges}}
...
\DIFadd{Figs.~\ref{fig:kde_fig0} and~\ref{fig:kde_fig1} show 1D and 2D density maps
constructed via Eqs.~\ref{eq:kde-1d} and~\ref{eq:kde-2d}, for two paired
parameters, for each of the MCs.}\@

\DIFaddend % Based on the analysis of a database of over 1500 star clusters, G10 reports two
...
%
There are two SMC clusters with very low metal abundance estimates, [Fe/H]$
{<-}2$ dex.
...
\DIFaddbegin \DIFadd{Internal errors show no biases present in our determination of fundamental
parameters, as seen in Sect.~\ref{sec:errors-fit}.
%DIF > 
}\DIFaddend % TOMADO DE BAAA58 v
The analysis of our results \DIFaddbegin \DIFadd{in Sect.~\ref{ssec:lit-values}}\DIFaddend , demonstrate that the
...
photometric systems\DIFdelbegin \DIFdel{. }\DIFdelend \DIFaddbegin \DIFadd{, in Sect.~\ref{ssec:db-values}. }\DIFaddend While the age differences in
this case are somewhat larger, they can be mostly explained by effects outside
the code.
% TOMADO DE BAAA58 ^
\DIFdelbegin \DIFdel{Internal errors show no biases present in our determination of fundamental
parameters.
}\DIFdelend 

We performed \DIFaddbegin \DIFadd{in Sect.~\ref{sssec:integ_photom_masses} }\DIFaddend a detailed comparative
...
combination of two of them -- is presented \DIFdelbegin \DIFdel{. }\DIFdelend \DIFaddbegin \DIFadd{in Sect.~\ref{ssec:kde_method}. }\DIFaddend This
...
%
An age-metallicity relation is derived \DIFaddbegin \DIFadd{in Sect.~\ref{ssec:amr} }\DIFaddend using the above
mentioned method\DIFaddbegin \DIFadd{, }\DIFaddend and the parameter values obtained by \texttt{ASteCA}, for
...
treat it the same way we treat the metallicity, age, reddening, and distance
parameters.
...
reddening parameters were fixed. In Table~\ref{tab:massclean-range} these
