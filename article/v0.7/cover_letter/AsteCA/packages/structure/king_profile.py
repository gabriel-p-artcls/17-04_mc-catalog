...
def fit_3P_King_prof(fd, radii_k, ring_dens_k, guess3):
    ...
    with warnings.catch_warnings():
        ...
        try:
            param_bounds = ([0., guess3[1] * 0.3, 0.],
                            [guess3[1], guess3[1] * 3., np.inf])
            ysigma = np.linspace(1., 0.1, len(radii_k))
            popt, pcov = curve_fit(lambda x, rc, rt, cd: kpf.three_params(
                                   x, rc, rt, cd, fd), radii_k, ring_dens_k,
                                   guess3, bounds=param_bounds, sigma=ysigma)
        except:
    ...
    if np.isfinite(pcov).all():
        e_rc = np.sqrt(pcov[0][0]) if pcov[0][0] > 0 else -1.
...
def main(clust_rad, field_dens, radii, rdp_points):
    ...
    if g.kp_flag:
        ...
        rt_guess, rc_guess = clust_rad, clust_rad * 0.5
        # Skip RDP points that are either: closer to the center and smaller
        # than the maximum density, or beyond the estimated cluster radius.
        max_i = np.argmax(rdp_points)
        bck_i = (np.abs(np.array(radii) - clust_rad)).argmin() + 1
        radii_k = radii[max_i:bck_i]
        ring_dens_k = np.array(rdp_points[max_i:bck_i]) - fd

        max_dens = max(ring_dens_k) /\
            (1 - (1 / np.sqrt(1 + (rt_guess / rc_guess) ** 2))) ** 2

        # Attempt to fit a 3P-KP, fixing *only* the field density.
        try:
            guess3 = (rc_guess, rt_guess, max_dens)
            cd, rc, e_rc, rt, e_rt = fit_3P_King_prof(0., radii_k,
                                                      ring_dens_k,
                                                      guess3)
            # Fit converged.
