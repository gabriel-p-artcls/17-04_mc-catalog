...
def pl_rad_dens(gs, radii, ring_density, field_dens, coord, clust_name,
    ...
    x_min, x_max = max(min(np.log10(radii)) - (max(np.log10(radii)) / 20.), 0), \
        max(np.log10(radii)) + (max(np.log10(radii)) / 20.)
    delta_total = max(np.log10(ring_density)) - np.log10(field_dens)
    delta_backg = 0.2 * delta_total
    y_min = -3.7 #np.log10(field_dens) - abs(np.log10(field_dens) * 0.1)
    y_max = max(np.log10(ring_density)) + abs(max(np.log10(ring_density)) * 0.1)
    # Set plot limits
    plt.xlim(x_min, x_max)
    plt.ylim(y_min, y_max)
    ax.minorticks_on()
    ...
    ax.plot(np.log10(radii), np.log10(np.array(ring_density) - field_dens),
            'ro-', zorder=3, label=texts[0])
    ax.plot(np.log10(radii), np.log10(ring_density),
            'ko-', zorder=3)
    # # Plot Poisson error bars
    # plt.errorbar(radii, ring_density, yerr=poisson_error, fmt='ko',
    #              zorder=1)
    # # Plot background level.
    ax.hlines(y=np.log10(field_dens), xmin=np.log10(min(radii)),
              xmax=np.log10(max(radii)), label=texts[1],
              color='b', zorder=5)
    ...
    if flag_3pk_conver:
        ...
        ax.plot(
            np.log10(radii),
            np.log10((kpf.three_params(radii, rc, rt, cd, 0.))),
    ...
    #               lw=4., zorder=4)
        ax.vlines(x=np.log10(rt), ymin=y_min, ymax=y_max,
                  label=texts[4], color='g')
        ax.vlines(x=np.log10(rc), ymin=y_min, ymax=y_max,
                  label=texts[3], color='g')
    ...
    ax.vlines(x=np.log10(clust_rad), ymin=y_min, ymax=y_max, label=texts[5],
              color='r')
    ...
    handles, labels = ax.get_legend_handles_labels()
    # use them in the legend
    ax.legend(handles, labels, loc='lower left', numpoints=2, fontsize=11)
