...
def pl_lum_func(gs, mag_data, y_ax, flag_no_fl_regs, lum_func, completeness):
    '''
    ...
    if flag_no_fl_regs is not True:
        ...
        # Combined field + cluster region
        plt.step(x_all, y_all, where='post', color='c', lw=1.,
                 label='$LF_{all}$', zorder=3)
    ...
    text = '$' + y_ax + r',_{c}\,\approx\,%0.1f$' % mag_peak
    ax.vlines(x=mag_peak, ymin=0., ymax=plt.ylim()[1], color='k',
              lw=1.5, linestyles='dashed', label=text, zorder=1)
    # Legends.
    leg = plt.legend(fancybox=True, loc='upper right', handlelength=1,
                     fontsize=10)
    # Set the alpha value of the legend.
    leg.get_frame().set_alpha(0.7)
...
def pl_integ_mag(gs, integr_return, y_ax, x_ax0, flag_no_fl_regs):
    ...
    if integr_return:
        ...
        if fl_reg_mag1[0].any() and fl_reg_mag2[0].any():
            ...
            x_min, x_max = min(min(cl_reg_mag1[0]), min(cl_reg_mag2[0])) - \
                0.2, max(max(cl_reg_mag1[0]), max(cl_reg_mag2[0])) + 0.2
            y_min, y_max = max(max(cl_reg_mag1[1]), max(cl_reg_mag2[1])) + \
                0.2, min(min(cl_reg_mag1[1]), min(cl_reg_mag2[1])) - 0.2
        plt.xlim(x_min, x_max)
